...
- 2026-08-04 — Streamlit Community Cloud's default Python version (3.14.6 at time of writing) has no `numpy==1.26.4` wheel, so `uv pip install` tries a from-source build and the app crash-loops at "Resolved N packages" indefinitely instead of surfacing a clear error — nothing in the visible log says "failed" or "OOM", it just re-provisions the machine every 30-45s. Fix is in the app's own Settings → Python version dropdown (not a repo file): pin to 3.12. Worth checking first if a Streamlit Cloud build ever seems to hang past a couple of minutes.
