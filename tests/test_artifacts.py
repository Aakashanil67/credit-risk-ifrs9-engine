...
def test_artifact_bundle_round_trips_model_contract_and_metadata(tmp_path):
    ...
    bundle = ArtifactBundle(
        model=ToyModel(),
        train_medians={"income": 150_000.0},
        category_dtypes={"contract": ["Cash loans", "Revolving loans"]},
        metadata={"profile": "application", "feature_names": ["income", "credit"]},
    )
    ...
    assert loaded.train_medians == {"income": 150_000.0}
...
def test_artifact_bundle_rejects_metadata_that_disagrees_with_the_model_schema(tmp_path):
    ...
    bundle = ArtifactBundle(
        ...
        train_medians={},
        category_dtypes={},
        metadata={"profile": "application", "feature_names": ["credit", "income"]},
    )
